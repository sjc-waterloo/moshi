...
wget http://gitlab.moshimori.com/ec/mmecjms/raw/develop/mmjms.sql
...
sed -i 's/52.187.31.73/'"ip"'/g' import.sql
sed -i 's/52.33.190.253/'"ip"'/g' import.sql
sed -i "s/mt1/$vHOST\/ec/g" import.sql
mysql -u root -h localhost -p $ACCOUNT < import.sql

psSetting=/var/www/html/$vHOST/mm/config/settings.inc.php:

sudo sed -i "s/'mm1'/'"$ACCOUNT"'/g" /var/www/html/$vHOST/mm/config/settings.inc.php
mv /var/www/html/$vHOST/mm /var/www/html/$vHOST/ec
sed -i 's/\/mt1/\/'"$vHOST"'\/ec/g' /var/www/html/$vHOST/ec/.htaccess
